    ...
    <div v-if="order" class="space-y-4">
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatOrderPayAmount(order, baseAmount) }}</p>
        </div>
        <div v-if="order.fee_rate > 0">
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.fee') }} ({{ order.fee_rate }}%)</p>
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatOrderPayAmount(order, feeAmount) }}</p>
        </div>
        <div>
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') }}</p>
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatOrderPayAmount(order) }}</p>
        </div>
        <div v-if="order.amount !== order.pay_amount">
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.creditedAmount') }}</p>
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatOrderCreditedAmount(order) }}</p>
        </div>
        <div>
...
import { statusBadgeClass, canRefund as canRefundStatus, formatOrderDateTime, formatOrderPayAmount, formatOrderCreditedAmount } from '@/components/payment/orderUtils'
