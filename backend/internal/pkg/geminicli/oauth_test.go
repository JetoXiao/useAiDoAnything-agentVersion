...
func TestBuildAuthorizationURL_UsesBuiltinSecretFallback(t *testing.T) {
	...
	if err == nil {
		t.Fatal("BuildAuthorizationURL() missing built-in secret should fail")
	}
	if !strings.Contains(err.Error(), GeminiCLIOAuthClientSecretEnv) {
		t.Errorf("error should mention %s, got %v", GeminiCLIOAuthClientSecretEnv, err)
	}
}
...
func TestEffectiveOAuthConfig_NoEnvSecret(t *testing.T) {
	...
	if err == nil {
		t.Fatal("missing built-in secret should fail")
	}
	if !strings.Contains(err.Error(), GeminiCLIOAuthClientSecretEnv) {
		t.Errorf("error should mention %s, got %v", GeminiCLIOAuthClientSecretEnv, err)
	}
