...
func TestReconcilePendingProviderOrdersBackfillsInfiniOrder(t *testing.T) {
	ctx := context.Background()
	client := newPaymentOrderLifecycleTestClient(t)

	user, err := client.User.Create().
		SetEmail("infini-reconcile@example.com").
		SetPasswordHash("hash").
		SetUsername("infini-reconcile-user").
		Save(ctx)
	require.NoError(t, err)
	...
	order, err := client.PaymentOrder.Create().
		...
		SetPayAmount(4.2857).
		SetFeeRate(0).
		SetRechargeCode("INFINI-RECONCILE").
		SetOutTradeNo("sub2_infini_reconcile").
		SetPaymentType(payment.TypeUSDT).
		SetProviderKey(payment.TypeInfini).
		SetPaymentTradeNo("ord_infini_reconcile").
		SetOrderType(payment.OrderTypeBalance).
		SetStatus(OrderStatusPending).
		SetExpiresAt(time.Now().Add(time.Hour)).
		SetClientIP("127.0.0.1").
	...
	registry := payment.NewRegistry()
	provider := &paymentOrderLifecycleQueryProvider{
		key: payment.TypeInfini,
		resp: &payment.QueryOrderResponse{
			TradeNo: "ord_infini_reconcile",
			Status:  payment.ProviderStatusPaid,
			Amount:  4.2857,
			Metadata: map[string]string{
				"currency":         payment.TypeUSDT,
				"infini_order_id":  "ord_infini_reconcile",
	...
	require.NoError(t, err)
	require.Equal(t, 1, recovered)
	require.Equal(t, "ord_infini_reconcile", provider.lastQueryTradeNo)

	reloaded, err := client.PaymentOrder.Get(ctx, order.ID)
	require.NoError(t, err)
	require.Equal(t, OrderStatusCompleted, reloaded.Status)
	require.Equal(t, "ord_infini_reconcile", reloaded.PaymentTradeNo)
	require.Equal(t, 30.0, userRepo.getByIDUser.Balance)
	require.Len(t, redeemRepo.useCalls, 1)
}
