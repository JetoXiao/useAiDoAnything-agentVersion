...
> **UseAiForMe 官方仅使用  `sub2api.org` 与 `pincc.ai` 两个域名。其他使用 UseAiForMe 名义的网站可能为第三方部署或服务，与本项目无关，请自行甄别。**
---
...
UseAiForMe 是一个 AI API 网关平台，用于分发和管理 AI 产品订阅的 API 配额。用户通过平台生成的 API Key 调用上游 AI 服务，平台负责鉴权、计费、负载均衡和请求转发。
...
<td valign="middle"><b><a href="https://shop.pincc.ai/">PinCC</a></b> 是基于 UseAiForMe 搭建的官方中转服务，提供 Claude Code、Codex、Gemini 等主流模型的稳定中转，开箱即用，免去自建部署与运维烦恼。</td>
</tr>
...
<td>感谢 丝绸API 赞助了本项目！ <a href="https://code.silkapi.com/register?aff=SUB2API">丝绸API</a> 是基于 UseAiForMe 搭建的中转服务，专注于提供 Codex 高速稳定API中转。</td>
</tr>
...
围绕 UseAiForMe 的社区扩展与集成项目：

| 项目 | 说明 | 功能 |
|------|------|------|
| ~~[Sub2ApiPay](https://github.com/touwaeriol/sub2apipay)~~ | ~~自助支付系统~~ | **已内置** — 支付功能已集成到 UseAiForMe 中，无需独立部署。详见 [支付配置指南](docs/PAYMENT_CN.md) |
...
通过 Nginx 反向代理 UseAiForMe（或 CRS 服务）并搭配 Codex CLI 使用时，需要在 Nginx 配置的 `http` 块中添加：
...
UseAiForMe 支持 [Antigravity](https://antigravity.so/) 账户，授权后可通过专用端点访问 Claude 和 Gemini 模型。
